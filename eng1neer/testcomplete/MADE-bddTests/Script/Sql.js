...
function setEnvironment(envt) {
  ...
  Project.Variables.dboServerURL =
    Project.Variables.dboServer +
    Project.Variables.sgbEnvironment +
    ".database.windows.net";
  Log.Checkpoint(
    "| Environment Set | - " +
      aqString.ToUpper(Project.Variables.sgbEnvironment),
  );
}
...
function sqlQueryNone(query) {
  ...
  AConnection.ConnectionString =
    ...
    ";" +
    "Pwd=" +
    Project.Variables.dboPass.DecryptedValue +
    ";";
  AConnection.Open();
...
function sqlRefreshDatabase(dbName) {
  ...
  var AConnection, recSet;
  var query = aqFile.ReadWholeTextFile(
    Project.Path +
      "\\Stores\\Files\\refresh_" +
      Project.Variables.varDbName +
      ".sql",
    aqFile.ctANSI,
  );
