...
function gppLogin() {
  ...
  if (equal(gpp.Auth.Exists, true)) {
    ...
    Log.Message(
      "LOG: user = " +
        Project.Variables.gppUsername +
        " | envt = " +
        Project.Variables.gppEnvironment,
    );
  }
...
function gppSearchMID(searchFor) {
  ...
  aqObject.CheckProperty(
    gppTransaction.ref_MID,
    "value",
    cmpEqual,
    Project.Variables.gppPaymentId,
  ); // M098A27047S96VZ2
  aqObject.CheckProperty(
    gppTransaction.ref_Acct,
    "value",
    cmpEqual,
    Project.Variables.gppAccount,
  ); // 013500010023398
...
function EventControls_OnLogError(Sender, LogParams) {
  ...
  if (equal(Test, Start)) {
    Log.Message(
      "TEST: failed to launch website on: " + aqTestCase.CurrentTestCase.Name,
    );
    Runner.Stop();
  }
}
